...
func BuildRequestObject(request Request) (*http.Request, error) {
	...
	if err != nil {
		err = errUtil.New("EncodingError", "Error constructing http request", err)
		return req, err
	}
...
func BuildResponse(res *http.Response) (*Response, error) {
	...
	if err != nil {
		err := errUtil.New("EncodingError", "Error reading the response body", err)
		return nil, err
	}

	if res.StatusCode < 200 || res.StatusCode > 299 {
		err = errUtil.New("UpstreamRespondedWithError", "Received error response from Upstream", errors.New(string(body)))
	} else {
		response = Response{
...
func (c *Client) Send(request Request) (*Response, error) {
	...
	if err != nil {
		err := errUtil.New("MakeRequestError", "Error making the request", err)
		return nil, err
	}
...
func EncodeBody(body interface{}) (*bytes.Buffer, error) {
	...
	if encodeErr != nil {
		err := errUtil.New("EncodingError", "Error encoding the request body", encodeErr)
		return nil, err
	}
